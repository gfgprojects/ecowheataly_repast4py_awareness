...
class Farm(core.Agent):
    ...
    def __init__(self, local_id: int, rank: int,my_params,my_place_params):
        ...
        self.ceo_age=my_params['young']
        self.ceo_gender=my_params['gender']
        ceo_gender_for_theta=1
        if self.ceo_gender=='M': ceo_gender_for_theta=0
        #self.environmental_awareness=(repastrandom.default_rng.uniform(low=50.0,high=100.0,size=1))[0]
        #self.environmental_awareness=100
        eta=0
        self.environmental_awareness=eta*(repastrandom.default_rng.random()+0.5*(90-self.ceo_age)/(90-18)+0.4*ceo_gender_for_theta)
        self.environmental_awareness=0
        if self.environmental_awareness<0: self.environmental_awareness=0
        self.policy_target_at_actual_level_of_awareness=False
        ...
        self.unconstrained_recipe_daly=0
        self.unconstrained_recipe_species=0
        ...
        self.constrained_recipe_daly=0
        self.constrained_recipe_species=0

        self.unconstrained_utility=0
        self.constrained_utility=0
        self.utility=0
        self.best_utility_type='any'       
        self.max_profit_Nitrogen_per_ha=0
        self.max_profit_Herbicide_per_ha=0
        self.max_profit_Insecticide_per_ha=0
        self.max_profit_per_ha=0
    ...
    def compute_unconstrained_production_inputs_with_utility(self):
        """Compute optimal profit-daly combination

        Starting from profit maximization, gradually decrease the target yield until utility is maximal.
        Derives the input levels for
        nitrogen, herbicide, and insecticide at maximum utility.
        """
        self.policy_unconstrained_inputs=[1,1,1]
        if True:
            if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                print("farm "+str(self.id)+" I am in rank "+str(self.rank)+" uid "+str(self.uid)+' inputs decision:')
                print('bar_y:',self.bar_y)
                print('wheat p:',self.p_w_ql)
                print('inputs p',self.p_x_i)
                print('s:',self.s_i)
                print('lambdas:',self.lambda_i)
                print('pol unconstr inp:',self.policy_unconstrained_inputs)
        ## Find the maximum profit yield
        ...
        for i in range(len(self.s_i)):
            if self.bar_s_i[i]*self.bar_y==0: print(self.province,self.altimetry,str(self.bar_s_i[i]))
            log_arg=((1+self.bar_s_i[i]-self.s_i[i])*self.bar_y-hat_y)/(self.bar_s_i[i]*self.bar_y)
            #print(log_arg)
            #if log_arg <= 0:
            #    log_arg=0.1
            hat_x=-1/self.lambda_i[i]*math.log(log_arg)
            ...
            hat_x_i.append(hat_x)
            if params.verboseFlag: print('x',str(i),'=',str(round(hat_x,2)))
        profit_per_ha=self.p_w_ql*hat_y-self.p_x_i[0]*hat_x_i[0]-self.p_x_i[1]*hat_x_i[1]-self.p_x_i[2]*hat_x_i[2]
        self.max_profit_per_ha=profit_per_ha
        self.max_profit_Nitrogen_per_ha=hat_x_i[0]
        self.max_profit_Herbicide_per_ha=hat_x_i[1]
        self.max_profit_Insecticide_per_ha=hat_x_i[2]


        ##find DALY at maximum profit
        if self.altimetry =='Montagna':
            self.hours_of_tractor_use_ha=round(15+0.1*hat_y*0.1,2) #remember hat_y 100kg
        if self.altimetry =='Collina':
            self.hours_of_tractor_use_ha=round(12.5+0.1*hat_y*0.1,2) #remember hat_y is in 100kg unit
        if self.altimetry =='Pianura':
            self.hours_of_tractor_use_ha=round(10+0.1*hat_y*0.1,2) #remember hat_y is in 100kg unit
        hours_of_tractor_use_ha=self.hours_of_tractor_use_ha
        MJ=100/0.2779*hours_of_tractor_use_ha
        #assigning tractor power
        self.A[0,2]=-MJ
        #assigning Nitrogen fertilizer. 0.46 is average Nitrogen concentration in commercial product
        self.A[1,2]=-hat_x_i[0]*0.46
        #assigning insecticide with active principle deltamethrin. 0.025 is deltamethrin concentration in commercial product
        self.B[2,2]=hat_x_i[1]*0.025
        #assigning herbicide with active principle 2,4-D. 0.6 is 2,4-D concentration in commercial product
        self.B[46,2]=hat_x_i[2]*0.6
        #LCA computation
        ...
        recipe_daly=daly_sum*365*24
        recipe_species=species_sum*100000

        ##compute utility at maximum profit
        utility=profit_per_ha-self.environmental_awareness*recipe_daly

        if True:
            if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                print("farm "+str(self.id)+" I am in rank "+str(self.rank)+" uid "+str(self.uid)+' inputs decision:')
                print('profit per ha:',profit_per_ha)
                print('DALY:',recipe_daly)
                print('awareness:',self.environmental_awareness)
                print('utility:',utility)


        ## decrease target yield until utility grows
        delta_hat_y=0.5
        decrease_hat_y=True
        while decrease_hat_y:
            hat_y-=delta_hat_y
            hat_x_i=[]
            for i in range(len(self.s_i)):
                if self.bar_s_i[i]*self.bar_y==0: print(self.province,self.altimetry,str(self.bar_s_i[i]))
            ...
            #if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
            #    print(profit_per_ha)
            if utility>=profit_per_ha-self.environmental_awareness*recipe_daly or profit_per_ha<100:
                decrease_hat_y=False
            else: 
                utility=profit_per_ha-self.environmental_awareness*recipe_daly
            #if tmp_counter>100:
                #decrease_hat_y=False

        #recompute values at maximum utility (step back the last execution of while)
        if self.environmental_awareness==0:
            hat_y+=delta_hat_y
        else:
            hat_y+=0.5*delta_hat_y
        ...
        recipe_daly=daly_sum*365*24
        recipe_species=species_sum*100000
        self.recipe_daly=recipe_daly
        self.recipe_species=recipe_species
        self.constrained_recipe_daly=recipe_daly
        self.constrained_recipe_species=recipe_species
        ...
        self.unconstrained_Insecticide_per_ha=hat_x_i[2]
        self.unconstrained_inputs=hat_x_i
        self.profit_per_ha=profit_per_ha
        self.unconstrained_utility=utility
        #self.profit_per_ha=self.p_w_ql*self.hat_y-self.p_x_i[0]*self.unconstrained_Nitrogen_per_ha-self.p_x_i[1]*self.unconstrained_Herbicide_per_ha-self.p_x_i[2]*self.unconstrained_Insecticide_per_ha
    ...
    def compute_constrained_production_inputs_with_utility(self):
        """Compute production inputs under mandatory policy constraints.
        ...
        #hat_y=fsolve(self._foc_residual,15)[0]
        hat_y=round(min(hat_y_vector),2)
        self.hat_y=round(min(hat_y_vector),2)
        self.constrained_hat_y=self.hat_y
        #if self.uid[0]==18 and self.uid[1]==0 and self.uid[2]==1:
        #    print(self.hat_y)
        ...
        self.recipe_daly=recipe_daly
        self.recipe_species=recipe_species
        self.constrtained_recipe_daly=recipe_daly
        self.constrained_recipe_species=recipe_species
    ...
    def decide_production_inputs(self):
        ...
        if True:
            if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                print("farm "+str(self.id)+" I am in rank "+str(self.rank)+" uid "+str(self.uid)+' inputs decision:')
                print('TRAC',self.hours_of_tractor_use_ha_after_policy)

    def decide_production_inputs_with_utility(self):
        #self.Nitrogen_per_ha=hat_x_i[0]
        self.Nitrogen_per_ha_after_policy=self.unconstrained_Nitrogen_per_ha *self.policy_multiplier_N
        #self.Herbicide_per_ha=hat_x_i[1]
        self.Herbicide_per_ha_after_policy=self.unconstrained_Herbicide_per_ha*self.policy_multiplier_H
        #self.Insecticide_per_ha=hat_x_i[2]
        self.Insecticide_per_ha_after_policy=self.unconstrained_Insecticide_per_ha*self.policy_multiplier_I
        self.hat_y_after_policy=self.hat_y*self.policy_multiplier_hat_y
        if self.altimetry =='Montagna':
            self.hours_of_tractor_use_ha=round(15+0.1*self.hat_y*0.1,2) #remember hat_y 100kg
            self.hours_of_tractor_use_ha_after_policy=round(15+0.1*self.hat_y_after_policy*0.1,2)
        ...
        if True:
            if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                ...
                print('-----production-----')
                print(self.policy_unconstrained_inputs)
                print('production',self.hat_y_after_policy)
                print(self.Nitrogen_per_ha_after_policy,self.Herbicide_per_ha_after_policy,self.Insecticide_per_ha_after_policy)
                print('TRAC',self.hours_of_tractor_use_ha_after_policy)
    ...
    def keep_or_change_policy(self,tmpmodel):
        ...
        if tmp_policies['payment/ha'].sum()==0:
            ...
            #if no policy is in place, adopt policy or continues with any
            else:
                prob_treshold_for_adoption=max(prob_tresholds)
                max_prob_idx=prob_tresholds.index(prob_treshold_for_adoption)
                best_candidate_policy=tmp_policies.index[max_prob_idx]
    ...
    def keep_or_change_policy_with_utility(self,tmpmodel):
        """Evaluate whether to adopt, keep, or abandon an agri-environmental policy.
        ...
        if tmp_policies['payment/ha'].sum()==0:
            ...
            #uncontrained profit
            self.compute_unconstrained_production_inputs_with_utility()
            unconstrained_profit_per_ha=self.profit_per_ha
            unconstrained_utility=self.unconstrained_utility
            unconstrained_daly=self.recipe_daly


            if True:
                if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                    print("farm "+str(self.id)+" I am in rank "+str(self.rank)+" uid "+str(self.uid)+' inputs decision:')
                    print('unconstrained_profit:',unconstrained_profit_per_ha)
                    print('unconstrained_utility:',unconstrained_utility)
                    print('unconstrained inputs',self.unconstrained_inputs)
                    print('max prof inputs',self.max_profit_Nitrogen_per_ha,self.max_profit_Herbicide_per_ha,self.max_profit_Insecticide_per_ha)
                    print()
            ...
            #find probabilities of adoption
            ##################
            #In this model we evaluate only one policy with one mandatory requirement
            #most of the code is no mere needed, but it was not cancelled.
            #as an example, the prob_tresholds are no more used.
            #Please check that the abm_input_policies has a single line



            policy_target_at_actual_level_of_awareness=self.policy_target_at_actual_level_of_awareness

            prob_tresholds=[]
            tmp_profit_loss=0
            for i,row in tmp_policies.iterrows():
                unconstrained_better_than_policy=False
                #tmp_beta=(repastrandom.default_rng.uniform(low=max([row['profit']-0.01,0.0]),high=row['profit'],size=1))[0]
                if row['profit']>=0:
                    tmp_beta=row['profit']+self.rnd_for_beta
                    tmp_profit_loss=round(tmp_beta*unconstrained_profit_per_ha,2)
                    ...
                    if row['i_type']=='mandatory': self.policy_unconstrained_inputs[2]=0
                    #make a vector that will be used by compute_constrained_production_inputs ..., but only the mandatory will be used
                    #policy can be specified in terms of the choices at maximim profit i.e. at awareness=0 or at the actual level of awareness
                    #if policy is specified at actual level of awareness
                    if policy_target_at_actual_level_of_awareness:
                        self.constrained_inputs=[self.unconstrained_Nitrogen_per_ha*(1-row['n_effect']),self.unconstrained_Herbicide_per_ha*(1-row['h_effect']),self.unconstrained_Insecticide_per_ha*(1-row['i_effect'])]
                        for i in range(len(self.constrained_inputs)):
                            if self.policy_unconstrained_inputs[i]==0:
                                if self.unconstrained_inputs[i]<self.constrained_inputs[i]:
                                    unconstrained_better_than_policy=True
                    #if policy is specified at zero awareness
                    else:
                        self.constrained_inputs=[self.max_profit_Nitrogen_per_ha*(1-row['n_effect']),self.max_profit_Herbicide_per_ha*(1-row['h_effect']),self.max_profit_Insecticide_per_ha*(1-row['i_effect'])]
                        for i in range(len(self.constrained_inputs)):
                            if self.policy_unconstrained_inputs[i]==0:
                                if self.unconstrained_inputs[i]<self.constrained_inputs[i]:
                                    unconstrained_better_than_policy=True
                    ...
                    #    print(self.policy_unconstrained_inputs)
                    #    print(self.constrained_inputs)
                    self.compute_constrained_production_inputs_with_utility()
                    if policy_target_at_actual_level_of_awareness:
                        tmp_profit_loss=unconstrained_profit_per_ha-self.profit_per_ha
                    else:
                        #tmp_profit_loss=self.max_profit_per_ha-self.profit_per_ha
                        tmp_profit_loss=unconstrained_profit_per_ha-self.profit_per_ha
                tmp_paym=row['payment/ha']
                tmp_admin_costs=row['admin_costs']/self.farm_acreage
                constrained_profit_per_ha=tmp_paym-tmp_profit_loss-tmp_admin_costs+unconstrained_profit_per_ha
                #if policy_target_at_actual_level_of_awareness:
                #    constrained_profit_per_ha=tmp_paym-tmp_profit_loss-tmp_admin_costs+unconstrained_profit_per_ha
                #else:
                    #constrained_profit_per_ha=tmp_paym-tmp_profit_loss-tmp_admin_costs+self.max_profit_per_ha
                    #constrained_profit_per_ha=tmp_paym-tmp_profit_loss-tmp_admin_costs+self.profit_per_ha
                constrained_utility=constrained_profit_per_ha-self.environmental_awareness*self.recipe_daly

                if True:
                    if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                        print("farm "+str(self.id)+" I am in rank "+str(self.rank)+" uid "+str(self.uid)+' inputs decision:')
                        print('unconstrained N',self.unconstrained_Nitrogen_per_ha,'constr N',self.constrained_Nitrogen_per_ha)
                        print('constraints',self.policy_unconstrained_inputs)
                        print('constrained inputs',self.constrained_inputs)
                        print('max profit',self.max_profit_per_ha)
                        print('payment:',tmp_paym,'profit loss',tmp_profit_loss,'admin cost',tmp_admin_costs,'unconstr profit',unconstrained_profit_per_ha,'constrain prof',constrained_profit_per_ha,'max prof',self.max_profit_per_ha)
                        print('DALY:',self.recipe_daly)
                        print('delta utility:',constrained_utility-unconstrained_utility)
                        print('constained utility:',constrained_utility)
                        print('unconstrained_better_than_policy:',unconstrained_better_than_policy)
                        print('chosen policy:',self.policy_adopted)
                #treshold_for_adoption=tmp_paym-tmp_profit_loss-tmp_admin_costs
                #prob_treshold_for_adoption=1/(1+math.exp(-0.3*(treshold_for_adoption)))
                #prob_tresholds.append(prob_treshold_for_adoption)
                #if self.uid[0]==18 and self.uid[1]==0 and self.uid[2]==1:
            ...
            #    print(self.policy_adopted)
            #    print(self.p_w_ql)
            if unconstrained_better_than_policy:
                profit_unconstrained_with_policy=tmp_paym-tmp_admin_costs+unconstrained_profit_per_ha
                unconstrained_utility_with_policy=profit_unconstrained_with_policy-self.environmental_awareness*unconstrained_daly
            else:
                profit_unconstrained_with_policy=constrained_profit_per_ha
                unconstrained_utility_with_policy=constrained_utility-1
            utilities_df=pd.DataFrame([unconstrained_utility,constrained_utility,unconstrained_utility_with_policy],index=['unconstrained_utility','constrained_utility','unconstrained_utility_with_policy'],columns=['utility'])
            utilities_df.sort_values(by=['utility'],inplace=True)
            utility_type=utilities_df.idxmax()
            best_utility_type=utility_type.iloc[0]
            self.best_utility_type=best_utility_type
            
            #Multipliers was set to one at the beginning. They need not be modified if the best_utility_type is unconstrained or unconstrained with policy
            #in the following lines we change them if best_utility_type is constrained
 
            if best_utility_type=='constrained_utility':
                #set policy adopted variable
                self.policy_adopted=tmp_policies.index[0]
                chosen_policy=tmp_policies.iloc[0]
                #rerun production with constraint
                if chosen_policy['n_type']=='mandatory': self.policy_unconstrained_inputs[0]=0
                if chosen_policy['h_type']=='mandatory': self.policy_unconstrained_inputs[1]=0
                if chosen_policy['i_type']=='mandatory': self.policy_unconstrained_inputs[2]=0
                #make a vector that will be used by compute_constrained_production_inputs ..., but only the mandatory will be used
                ...
                if best_utility_type=='unconstrained_utility_with_policy':
                    self.policy_adopted=tmp_policies.index[0]
                else:
                    self.policy_adopted='any'
            if True:
                if self.uid[0]==0 and self.uid[1]==0 and self.uid[2]==1:
                    print('unconstrained_utility_with_policy:',unconstrained_utility_with_policy)
                    print('policy adopted:',self.policy_adopted)
                    print('utility type:',utility_type.iloc[0])
                    print('=====================')
                    print(self.uid)
                    print(self.policy_adopted)
                    print(self.s_i)
    ...
    def harvest(self):
        ...
        #self.harvested_y=round((repastrandom.default_rng.normal(loc=0.62*self.hat_y_after_policy,scale=self.risk,size=1))[0],2)
        self.harvested_y=0.68*self.hat_y_after_policy
