...
#    def __init__(self, comm: MPI.Intracomm, params: Dict):
    def __init__(self, comm: MPI.Intracomm):
        ...
        policies=all_policies.loc['sra20'].to_frame().T
        ...
        distributions_scaling=pd.read_csv('scaling.csv')
...
#print(tmp_prov_params)
        ...
        if artificial_farms_file_exists:
            ...
            for i in range(artificial_farms.shape[0]):
                ...
                scaling_factor_f=distributions_scaling.loc[(distributions_scaling['province']==province) & (distributions_scaling['altimetry']==altimetry),'scale_f'].values[0]
                scaling_factor_w=distributions_scaling.loc[(distributions_scaling['province']==province) & (distributions_scaling['altimetry']==altimetry),'scale_w'].values[0]
                ...
                for i in range(n_farms):
                    ...
                    while tmp_farm_acreage < farm_acreage_distribution.ppf(tmp_prob_f) or tmp_farm_acreage > scaling_factor_f*farm_acreage_distribution.ppf(0.99) or tmp_farm_acreage<0:
                        tmp_farm_acreage=round(scaling_factor_f*farm_acreage_distribution.rvs(size=1)[0],2)
                    tmp_wheat_acreage=round(scaling_factor_w*wheat_acreage_distribution.rvs(size=1)[0],2)
                    while tmp_wheat_acreage < wheat_acreage_distribution.ppf(tmp_prob_w) or tmp_wheat_acreage > scaling_factor_w*wheat_acreage_distribution.ppf(0.99):
                        tmp_wheat_acreage=round(scaling_factor_w*wheat_acreage_distribution.rvs(size=1)[0],2)
...
#        print(self.context.non_bounded_projs)
       ...
       #create the file to log farms data
        ...
        self.agent_logger = logging.TabularLogger(comm,output_file_name, ['tick','(id,type,rank)','farm code','province','altimetry','target_yield(kg)','crop_acreage','farm_acreage','yield(kg)','production(kg)','hours_of_tractor_use','N/ha(kg)','herbicide','insecticide','daly','nspecies','policy_adopted','best_utility_type'],delimiter=',')
       #create the file to log aggregate data
        ...
        print("=========perform italian production=========")
        ...
        #self.updatePolicies()
        print("======compute italian production========")
    ...
    def performItalianProductionSystemModel(self):
        ...
        if rank>0:
            ...
            for farm in self.context.agents(agent_type=0):
                farm.compute_unconstrained_production_inputs_with_utility()
                #farm.updateWheatPrice(self)
                farm.decide_production_inputs_with_utility()
    ...
    def updatePolicies(self):
        ...
        if rank>0:
            for farmer in self.context.agents(agent_type=0):
                farmer.keep_or_change_policy_with_utility(self)
